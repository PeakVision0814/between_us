...
begin
  ...
  if p_delete then
    ...
    from candidates
    join deleted_users
      on deleted_users.id = candidates.id
...
revoke all on function private.cleanup_stale_unverified_email_otp_users(interval, integer, boolean) from public;
grant execute on function private.cleanup_stale_unverified_email_otp_users(interval, integer, boolean) to service_role;

comment on function private.cleanup_stale_unverified_email_otp_users(interval, integer, boolean) is
  'Preview or delete stale email OTP auth.users rows that are unverified, never signed in, older than the grace period, and have no business-table references. Deleting auth.users cascades to public.profiles.';
