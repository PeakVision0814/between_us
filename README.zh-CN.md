...
当前产品假设会刻意收缩：先验证“想说的时候留一句话或一个小片刻，再把重要日子一直放在眼前”
这个轻量闭环，确认它值得被自然打开，而不是变成任务或日记。

## 产品焦点

- 构建一个真正的移动 App，而不是网页外壳。
- 把 MVP 收敛到一个低压力闭环：轻量片刻与重要日期。
...
- Home 展示两个人的共享概览、最近一条片刻预览、下一个重要日期，以及唯一主动作。
- Moments 展示轻量共享片刻流，而不是日记本。
- Dates 展示核心纪念日及其倒计时价值。
- Settings 提供 MVP 所需的最小个人设置：语言、主题、时区、通知预览。
