...
- 登录支持邮箱 / 手机号 + 密码 / 验证码双模式；注册与登录分离，手机号验证码真实送达仍依赖 Supabase SMS Provider 配置
- 注册在验证码验证通过后同页完成密码、昵称、性别；历史资料缺口用户仍通过昵称、性别、生日补全页进入
...
登录、邀请、共享同步、Row Level Security、个人资料闭环、单人态能力白名单、多语言基础设施第一阶段和双人空间退出机制 MVP 已进入当前基础能力。手机号 OTP 目前只完成前端入口、状态流、11 位大陆手机号校验、`+86` 转换和 Supabase Phone OTP 调用边界；真实短信送达依赖私测前配置 Supabase SMS Provider 或 Send SMS Hook。进入敏感数据或私测 Beta 前，建议先用两个真实账号手工验收邀请配对、退出空间和语言偏好恢复；手机号真实收码验收需等短信服务配置完成后再做。
